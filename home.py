...
if not st.session_state["authenticated"]:
    ...
    # ===== زر تحديث البيانات قبل تسجيل الدخول =====
    st.markdown("### 🔄 تحديث البيانات من Google Sheets")
    if st.button("🔁 تحديث البيانات الآن"):
        load_user_data.clear()  # مسح الكاش
        load_data.clear()       # مسح الكاش للبيانات العامة أيضًا (اختياري حسب الحاجة)
        st.success("✅ تم تحديث البيانات بنجاح! الرجاء إعادة تحميل الصفحة إذا لم تظهر التغييرات.")
        st.stop()  # إيقاف التنفيذ لتجنب عرض نموذج الدخول مباشرة بعد الضغط
